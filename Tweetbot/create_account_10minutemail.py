      ...
      if check_notnow != True:
        ...
        if check_verification == True:
          ...
          if check_phone == True:
            ...
            value_region = driver.find_elements_by_xpath('//select[@id="SELECTOR_8"]/option')
            ...
            print()
            print('Select an existing region, select it by typing the letter on the right')
            region_code = input('Enter code region: ')
            region_code = region_code.upper()
            try:
              Select(driver.find_element_by_id('SELECTOR_8')).select_by_value(region_code)
            except NoSuchElementException:
              time.sleep(3)
              Select(driver.find_element_by_id('SELECTOR_8')).select_by_value(region_code)
...
def minutemail(a,password):
  # ------- READ INPUT FROM CLI -------
  show('Twibot Create Account Email 10minutemail by Nizar', 'v1.0.0')
  print()
