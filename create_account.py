...
from Email import runEmail, getEmail, getCountSeconds, getCountBox, getCodeVerifTwitter, clearCookie
      ...
      if check_notnow != True:
        ...
        if check_verification == True:
          ...
          if check_phone == True:
            ...
            value_region = Select(driver.find_element_by_id('SELECTOR_4'))
            for region in value_region.option:
            ...
            try:
              Select(driver.find_element_by_id('SELECTOR_4')).select_by_value(region_code)
            except NoSuchElementException:
              time.sleep(3)
              Select(driver.find_element_by_id('SELECTOR_4')).select_by_value(region_code)
            ...
            if check_verification == True:
              ...
              def verif_code():
                ...
                while skip != True:
                  ...
                  time.sleep(2)
                  if code == '1':
                    click_object('/html/body/div/div/div/div[1]/div[2]/div/div/div/div/div/div[2]/div[2]/div/div/div[2]/div[2]/div/div/div[3]/div/div/div/div/span/span[2]')

                    time.sleep(2)

                    click_object('/html/body/div/div/div/div[1]/div[3]/div/div/div/div[2]/div[3]/div/div/div/div[2]')
  ...
  while i <= a:
    try:
      ...
      if email != False:
        ...
        print(f'Processing {str(i)}', end='\r')
...
def main(argv):
  # ------- READ INPUT FROM CLI -------
  description = 'Twitter Create Account Bot by Nizar'
  version = 'v1.0.0'
  show(description, version)
  print()
